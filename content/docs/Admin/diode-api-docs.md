...
Messages that start with "/" are [**chat commands**](https://support.diode.io/article/pgsegq2srl-diode-drive-chat-commands). Chat commands and the automated responses to them don't send any data to the channel members, only the sender can see them, and they disappear after leaving the page.
